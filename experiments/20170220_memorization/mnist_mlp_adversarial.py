...
with du.trial.run_trial(trial_name=trial_name) as trial:
    ...
    probe_hiddens = 2
    probe_critic_initial_steps = 100
    ...
    if 0:
        ...
        probe_dim = 512
        train["probe_target"] = np.random.uniform(
            low=-1,
            high=1,
            size=(train_size, probe_dim)).astype("float32")
        probe_target = tf.placeholder(tf.float32, shape=[None, probe_dim])
        probe_step_factor = 0.1

        if 0:
            def probe_cost_fn(h):
                return tf.reduce_mean(tf.square(h - probe_target))

        if 1:
            def probe_cost_fn(h):
                return tf.reduce_mean(tf.abs(h - probe_target))
    ...
    with tfu.variable_scope("mlp"):
        h = tfu.affine(h, hidden_size, name="fc1")
        fc1 = h = tf.nn.relu(h)
    ...
    with tf.name_scope("mlp_probe_opt"):
        ...
        for epoch_idx in range(num_epochs):
            ...
            with du.timer("train"):
                for train_batch in train_batches:
                    ...
                    if 0:
                        _, _, summary = sess.run([mlp_probe_train_step,
                                                  probe_train_step,
                                                  probe_summary_op],
                    ...
                    if 1:
                        for _ in range(probe_critic_steps
                                       if batch_idx > 0
                                       else probe_critic_initial_steps):
                            sess.run([probe_train_step],
                                     feed_dict={x: train_batch["x"],
                                                probe_target: train_batch["probe_target"],
                                                keep_prob: 1.0})

                        _, summary = sess.run([mlp_probe_train_step,
                                               probe_summary_op],
                                              feed_dict={x: train_batch["x"],
                                                         probe_target: train_batch["probe_target"],
                                                         keep_prob: 1.0})
                    ...
                    _, summary = sess.run([mlp_train_step,
                                           train_summary_op],
                                          feed_dict={x: train_batch["x"],
                                                     y: train_batch["y"],
                                                     keep_prob: 1.0})
