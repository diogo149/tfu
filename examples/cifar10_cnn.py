...
with tf.variable_scope("mlp"):
    h = tfu.conv2d("conv1", h, num_filters=96)
    h = tf.nn.relu(h)
    h = tfu.conv2d("conv2", h, num_filters=96)
    h = tf.nn.relu(h)
    h = tfu.max_pool2d(h)
    h = tfu.conv2d("conv3", h, num_filters=192)
    h = tf.nn.relu(h)
    h = tfu.conv2d("conv4", h, num_filters=192)
    h = tf.nn.relu(h)
    h = tfu.conv2d("conv5", h, num_filters=192)
    h = tf.nn.relu(h)
    h = tfu.max_pool2d(h)
    h = tfu.conv2d("conv6", h, num_filters=192)
    h = tf.nn.relu(h)
    h = tfu.conv2d("conv7", h, num_filters=192, filter_size=(1, 1))
    h = tf.nn.relu(h)
    h = tfu.conv2d("conv8", h, num_filters=10, filter_size=(1, 1))
